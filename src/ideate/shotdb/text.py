...
# configs/shot_lists/poc_v1.yaml labelled `verdict: bad, fault: locked mode;dud trip` when the
# operators had recorded the opposite. Its whole record (run 20180227) reads:
...
# build has already been read. Measured on configs/shot_lists/poc_v1.yaml: 8 of its 200 shots, from
# 4 run days (20170307, 20220630A, 20240626B, 20240821A), carry them in mp_purpose; nothing else in
# the human tier does, and sql/logs.jsonl has none in any of its 53,179 records. U+FFFD is the
# honest replacement -- "a character was here that could not be decoded" -- and it changes no
# classification: _prose_ratio already dropped these tokens, since neither a lone surrogate nor
# U+FFFD is str.isalpha().
