...
def load_cfg() -> dict:
    """The `retrieval:` block of configs/retrieval.yaml.
    ...
    for key, cast in (
        ...
        if key not in r:
            raise KeyError(f"configs/retrieval.yaml: retrieval.{key} is missing")
        out[key] = cast(r[key])
    return out
...
def rerank(
    ...
    `dedup_threshold` and `decay` default to configs/retrieval.yaml's values, read when they are
    not given -- not to numbers written here. A literal default (0.7) sat beside a YAML tuned to
    0.9, so a direct call silently reranked with a decay the config had abandoned.
...
# prints in the unit configs/ declares, unscaled. An unverified unit never gets a prefix invented
# for it.
...
def units() -> dict[str, str]:
    """Quantity -> the unit the registry declares, straight from configs/ -- never a unit guessed
    here. Several are honestly `[?]` (the line-integrated density, the neutron rate) and print
    that way; an unverified unit is not silently upgraded to a plausible one. Shared with
