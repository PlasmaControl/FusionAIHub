...
def get_events(
    ...
    * `observed` -- detectors ran over (part of) the window. An empty `events` here is a real
    ...
    # `database` joins `forecast` and `text` here (task L-D1): a curated table names a shot,
    # it does not observe one, so a shot whose only rows come from a spreadsheet must not
    # come back `status == "observed"` -- which is the whole error class the fourth list
    # exists to prevent, arriving through the status field instead of through the list.
    n_observed_rows = 0 if all_rows is None else int(
        (~all_rows["evidence_kind"].isin(("forecast", "text", "database"))).sum()
    )
    ...
    events = [
        r for r in rows
        if r.get("evidence_kind") not in ("forecast", "text", "database")
    ]
