"""Phase-A FSQ bottleneck for the fast-TS (filterscopes) codec.
...
``cfg.fsq_dim`` — fields the :class:`~ignite.config.FastTSCodecConfig` carries with the same
names and meaning as :class:`~ignite.config.SpectroCodecConfig`. So the fast-TS quantizer
REUSES :class:`~ignite.quantizer.SpectroQuantizer` verbatim (straight-through FSQ + the
Genie/LFQ entropy anti-collapse term) — a thin, documented alias, exactly as
:class:`~ignite.video_quantizer.VideoQuantizer` does for video (docs/IGNITE_DESIGN.md §7).
...
class FastTSQuantizer(SpectroQuantizer):
    """FSQ bottleneck for the fast-TS codec — same STE-FSQ + entropy machinery as spectro.
    ...
    ``FastTSCodecConfig``, which exposes the identical ``d_model`` / ``fsq_levels`` /
    ``fsq_dim`` / ``diversity_weight`` fields the parent reads).
    """

    def __init__(self, cfg: FastTSCodecConfig) -> None:  # noqa: D401 (thin init)
