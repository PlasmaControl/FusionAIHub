...
def _qh_coverage(
    ...
    The track span is the HULL of the blocks the tracker ran on. It is the
    one span here that is a union rather than an intersection, because a
    track seen on any block is a track; the blocks of one shot come off
    digitisers that overlap almost entirely, so the hull is not a material
    over-claim, and it is intersected with three narrower spans anyway.
    """
    if not runs:
        return coverage.UNKNOWN, QH_NEEDS_TRACKS
    if reference is None or "elm_clock" in skipped:
    ...
    cov = coverage.intersect([
        tracks_span, reference.t_cov, spans["pinj_total"], ip_flattop[0],
    ])
    if not (math.isfinite(cov[0]) and math.isfinite(cov[1])):
...
def finish_shot(
    ...
    *,
    ...
    qh_cov, why = _qh_coverage(runs, reference, res.skipped, spans, ip_flattop)
    if why:
