...
def envelope(y, t_s, env_ms=1.0)  # vectorised bincount; ==omnimode to 1e-12; isolated ≥3× faster + one call per shot (structural pin); whole detector <1 s/shot [amended 2026-09-07 post-critic; was: "≥10× faster (timing test)"]
...
```
Cost model: ≈15.2 core-s and ≈379 tiles per shot → 0.84 s A100 GPU → balance 18 cores/GPU. **Production scope is the 500-shot `recommender_v1` list plus a targeted rare-phenomenon extension of ≤ 200 masks-only shots** ≈ 10–15 min for the 500 on 4 A100s; the full 16,909-shot corpus is iteration 4. [amended 2026-09-07 post-critic: 500 + ≤ 200 extension; was: "2,000 shots ≈ 25–40 min on 4 A100s"] The `--array=0-7%2` width above is a planning figure, to be re-sized from the 20-shot **cold compute-node** pilot before production (plan §8).

## A6. Tasks (dependencies 1→{2,3,7}→4→{5,6}→8→9→{10,11}→12→13→14→15→16(user)→17→18)
1 schema+config+append_index(keys) [451+12 green] · 2 unet vendored + pin_unet golden (max_abs_diff 0.0) · 3 channels+masks [smoke on 198658 reproduces A1 table] · 4 tracks [synth exact; fast merge == literal] · 5 transients+ELM · 6 heuristics [198658: 45±3 sawteeth, **69±5 ms**, per-shot **< 1 s** binding (0.34–0.39 s measured; isolated `envelope` ≥3× + one-call pin); L→H on ≥3/10 H-mode shots — measured 9/10, with the **trend-gated hold withdrawn** and the stationary-baseline/uncalibrated-`LH_HOLD_FRAC`/inherited-NBI-bit deficits disclosed (plan §6)] [amended 2026-09-07 post-critic; was: "45±3 sawteeth, 76±5 ms, <1 s"] · 7 lexicons+text_weak [**acceptance replaced 2026-09-07 post-critic; was: "≥1 sawtooth hit on ≥5% of 200 shots"** — a poor success metric that would force genuine prevalence upward. New criterion: **source-faithful hits** (word-boundary matching with decimals treated as intra-token; shot scope read from `sql/logs.jsonl`; run scope reported separately and never counted as shot-scope evidence) **+ manually audited precision on a 50-hit sample**. Measured shot-scope prevalence over the 500 is **reported, not tuned**: sawtooth 3/500 = **0.6%** (an honest MISS against the old criterion), elm 55 (11.0%), tearing 36, lh 12, detachment 12, qh 10, eho 5, rwm 5, ae 3, fishbone 3; run scope elm/rwm 99.4%, tearing 49%, lh 42%, sawtooth 14.6%] · 8 windows+resolve_events+namespace/run edits [full suite green; **the resolver timebase is float64 SECONDS** — the axis every other feature source uses; an `ms` axis would be a silent ×1000 against the waveform axis `models/base.py` passes. [amended 2026-09-07 post-critic: contract stated explicitly; the erroneous "ms" wording lived only in the L8 task brief, never in this spec]] · 9 tokeye_masks.py [`--limit 5 --device cpu` end-to-end] · 10 jobstats_check.py [parses old dataset.json → 23/6/1/97, exits 1] · 11 sbatch + 20-shot pilot [measure tiles/s, RSS, util; size knobs] · 12 pilot ablation norm/zoom vs aemodes annotations [one-page report] · 13 production **500 + ≤ 200 targeted extension** [jobstats gate per plan §13 — CPU-only jobs on CPU/CPU-mem, production GPU jobs on all four ≥70%, pilots exempt but reported; the **pilot → gate → production stop rule is binding**. Acceptance is **complete evidence products** — masks, tracks, heuristics and events with explicit per-source completion and coverage via `events/<shot>_sources.parquet`: "≥95% have a file" is insufficient if the files hide skipped sources. Canonical Ip/features must be wired for QH and counter-injection handling (the pipeline currently skips these via `NO_IP`/`NO_FLATTOP`)] [amended 2026-09-07 post-critic; was: "13 production 2,000 [jobstats gate; ≥95% events files]"] · 14 priors+candidates [≥50 candidates/phenomenon; text ceiling] · 15 render+sheet [7 folders × 120 chunks; 10 PNGs each hand-inspected for legibility and correct overlays; selection reasons and scores must not bias judgement; **this task also owns the `analyze` events-overlay figure of plan §11.2** (`annotate/render.py` + a one-line `analyze` hook), which was left unowned when `analyze.py` was off-limits in L9] [amended 2026-09-07 post-critic: overlay ownership assigned] · 16 user annotates [≥60 labelled, ≥15 positives per trained phenomenon] · 17 annotate_train + gbdt runner + model folders [OOF AUROC vs permutation & text-only baselines; golden vs sklearn 1e-12] · 18 infer end-to-end + docs [existing groups untouched; `source=model` events].

## A7. Tests (≈180) — highlights
Trailing-NaN strip and interior-NaN raise; fs from span; prep == `ae.transform.compute_stft`; zoom axis 0.12207·k; tile/stitch identity & overlap averaging; pack/unpack at every T mod 8; synth_mask analytic descriptors (EHO 3 tracks n_harmonics=2; fishbone chirp −0.78±0.01 r²>0.99; pickup row; ELM period 15.36 ms); merge fast-path == literal port on 200 random sets; envelope == omnimode 1e-12; **isolated `envelope` ≥3× faster plus a one-call structural pin** (the detector computes it once per shot) under a binding per-shot < 1 s budget [amended 2026-09-07 post-critic; was: "≥10× faster", a figure scoped to the whole detector rather than the isolated call]; sawtooth rejects same-sign ELM impostor; lh returns [] without NBI; NaN channels ≥8 don't poison mean; actuator hysteresis + counter-injection sign; text word-boundary (" nt " ≠ "want"); FEATURE_NAMES frozen; TEXT_ONLY_CEILING for every prior; render uses Figure/Agg (pyplot monkeypatched to raise); sheet mismatch raises; gbdt hand arithmetic + golden; runner feature-name mismatch raises; adapters `card_discrepancies == []`.
