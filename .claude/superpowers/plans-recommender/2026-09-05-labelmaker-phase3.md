...
- [ ] `outputs/labelmaker/ae/README.md`: the numbers, both transforms side by side, the six figures, and a **verdict** line. **Nathan, 2026-09-05: the hand annotations under-count the real AE activity, so activity the mask finds outside an annotation is expected and acceptable - false positives against the annotation are not a failure.** Therefore: GO if, under the better transform, recall of annotated-active frames >= 0.70 at some occupancy threshold AND the per-shot recall is >= 0.5 on at least 80% of the 180 shots (the mask must find what the humans found, shot by shot, not only in aggregate); precision, AUROC and the unannotated fraction are REPORTED (they say how much more the mask finds) but never fail the verdict. NO-GO otherwise with the failing number named. Do not soften a NO-GO. Record the chosen transform as the answer to spec section 7 question 3 and append the decision to the spec's section 5.8/7 in the same commit.
...
**Preconditions:** Task 6 GO (met 2026-09-05: `tokeye` transform; pooled recall of annotated frames 0.912 at occupancy 0.01, 172/180 shots >= 0.5). **Task 6 also showed why the raw mask cannot be the label as-is:** 61.3% of all frames are called active at 0.01 (annotation: 21.0%), precision 0.313, and the six figures show the cause - broadband vertical streaks (ELM-like transients) leak into the coherent channel and lift the band occupancy to 0.05-0.3 for hundreds of ms on shots like `176042_train`, while genuine modes sit at 0.1-0.5 and quiet baseline at ~0.01. The construction below therefore cleans the mask before thresholding.

**Mask cleaning (decided by the controller from Task 6's figures, 2026-09-05):**
1. `coherent = sigmoid(ch0) >= 0.2` AND `transient = sigmoid(ch1) >= 0.2`; the mode mask is `coherent & ~transient` (channel 1 is tokeye's transient channel; a pixel the model also calls transient is not a coherent mode).
2. Temporal persistence: a coherent mode lasts; a streak does not. Apply a binary opening along the time axis with a length of `PERSIST_FRAMES = 20` (5.1 ms at 0.256 ms/frame) to the cleaned mask, per frequency bin. Record the value; it is a rule, not a fit.
3. Notch as before (bin lit in > threshold of frames), applied before occupancy.
4. Threshold rule: recompute pooled recall, precision and predicted-positive fraction against the annotation for the cleaned mask at occupancy thresholds {0.005, 0.01, 0.02, 0.03, 0.05, 0.08, 0.1}; choose the LOWEST threshold at which the predicted-positive frame fraction is <= 0.40 while pooled recall stays >= 0.80; if no threshold satisfies both, choose the one with recall >= 0.80 and the smallest positive fraction and say so. Report the full table in `dataset.json` and the README. Also report how much the cleaning alone (before thresholding) reduced the active-pixel count on `176042_train` and `175978_train`.
5. Human-review item carried from Task 6: `176526_train` loses 124.0-127.9 kHz in `r0` to the notch (inside the AE band). Write it into `notch_review.json` with a `needs_review: true` flag and exclude that channel's notch on that shot from the label until Nathan looks (use the other three channels' notches); list every shot where the notch removes bins below 190 kHz the same way.
...
**Label construction, revised 2026-09-05 after Nathan's note that the annotations under-count:** the primary frame label is the **mask alone** - `active[f] = band occupancy of the notched coherent mask >= threshold_from_task6` - because coherent activity in 80-250 kHz on the CO2 chords, with receiver lines notched, is AE activity by construction and the mask finds more of it than the annotators marked. The annotation (LFM excluded) is kept as a second array `annotated[f]` for evaluation and for a comparison label set `active_and_annotated = active & annotated` (the step_1_make_semantic construction). Task 7b trains on `active` and reports validation metrics against both `active` and `annotated`; the card says which label the shipped model was trained on.

- [ ] `scripts/labelmaker/ae_dataset.py` (tokeye venv or phase3 venv with tokeye installed non-editable; a GPU sbatch like Task 6's, saving BOTH sigmoid channels per shot as uint8 probabilities x 255 under /scratch/gpfs/EKOLEMEN/nc1514/labelmaker/ae/masks/): for the 180 shots, using the `tokeye` transform (compute_stft per channel, per-array standardisation as tokeye's model_infer) and the cleaned mask above: (1) notch: sweep thresholds {0.5, 0.6, 0.7, 0.8, 0.9} and record per shot which bins each removes; choose the smallest threshold at which no bin inside an annotated AE window's centroid +-3 bins is removed on more than 2 shots; write `notch_review.json` (per shot: bins removed, their kHz, occupancy) and `notch_review.png` (a heat map shot x bin of removed bins); (2) frame labels: `active[f] = mask occupancy in band >= threshold_from_task6` (mask alone, see above), `annotated[f]` = `label_1|label_2|label_3|label_4` resampled to the frame grid (LFM excluded), `freq_khz[f]` = intensity-weighted centroid over masked band pixels (NaN where inactive); (3) write per-shot `.npz` under `/scratch/gpfs/EKOLEMEN/nc1514/labelmaker/ae/dataset/<shot>.npz` with `spec` (4, 348, 7820) float16 band-restricted standardised spectrogram, `active` (7820,) uint8, `annotated` (7820,) uint8, `freq_khz` (7820,) float32, `notched_bins`, `split` ('train'/'valid' from the pickle's own lists), and a `dataset.json` manifest (counts, class balance, the thresholds, the transform, git sha).
- [ ] Hermetic unit tests for the pure functions in `tests/labelmaker/test_ae_labels.py`: `clean_mask(coherent, transient)`, `persist_open(mask, frames)` (a 19-frame blip disappears, a 20-frame run survives, a streak spanning all bins for 3 frames disappears), `band_occupancy(mask, lo_bin, hi_bin)`, `notch_bins(occupancy, threshold)`, `centroid_khz(spec, mask, bin_khz)`; put them in `src/labelmaker/ae/labels.py` (numpy/scipy.ndimage only, no torch) so the dataset script and, later, the adapter import them.
...
**Label decision after Task 7a (controller, 2026-09-05):** Task 7a's threshold rule could not be satisfied - the cleaned mask calls 53.5% of frames active at recall 0.864 of the annotation (21.0% of frames), and the transient veto plus persistence opening removed a third of the pixels without changing that trade-off. So neither "mask alone" nor "mask AND annotation" is the training label. The activity target is **three-way**, built at load time from the dataset's `active` (mask) and `annotated` arrays:

| frame | target | loss weight |
|---|---|---|
| `annotated & active` | 1 (confirmed AE, localised by the mask) | 1 |
| `~annotated & ~active` | 0 (nothing coherent, nothing annotated) | 1 |
| `active & ~annotated` | ignored (the under-count region Nathan described: possibly AE, unlabelled) | 0 |
| `annotated & ~active` | ignored (annotation the filter did not confirm) | 0 |

The model is free to call the ignored frames active without penalty, which is exactly the "more false positives are fine" instruction, while never being taught that mask-only frames are negatives. The frequency target is the mask centroid on frames with target 1 only. Comparison runs train on `active` alone (mask-only label) so the effect of the three-way target is measured, not assumed. Evaluation on the 60 validation shots reports, for every run: recall of `annotated`, precision vs `annotated`, predicted-positive fraction, and frequency MAE on `annotated & active` frames.

**Notch decision:** Task 7a's sweep picked 0.5 because every threshold passed its protected-bin test and the rule said "smallest". A bin lit in half of a 2 s record can be a real long-lived mode, so the label uses **0.8** (Task 6's first-pass value; 13 shots notch anything, max 7 bins). Rebuild `active`, `freq_khz`, `notched_bins` and `notch_excluded` from the saved masks (no GPU needed) with notch 0.8 before training, keep the withheld-notch review list, and record the change in dataset.json.
...
- [ ] `scripts/labelmaker/ae_train.py`: Lightning-free plain torch loop (the phase3 venv has no Lightning; do not add it): AdamW lr 1e-4 wd 1e-4, cosine to 1e-6 over 30 epochs, bf16 autocast, batch 16 windows, early stop on validation loss patience 5, checkpoint the best; split by shot from the manifest; a DataLoader with `num_workers` = the allocated CPUs and float16 -> float32 on the GPU (Tasks 6 and 7a were host-I/O bound); validation metrics each epoch: recall/precision/predicted-positive fraction against `annotated`, frame AUROC against the three-way target's labelled frames, and frequency MAE in kHz on `annotated & active` frames; three runs as array tasks: `--target threeway --loss sce` (primary), `--target threeway --loss bce`, `--target mask --loss sce`; each saves `ae_seldnet_<target>_<loss>.pt` (state_dict + config) under `/scratch/gpfs/EKOLEMEN/nc1514/labelmaker/models/d3d_ae_activity_seldnet/` with `training.json`.
- [ ] `ae_train.sbatch`: `gpu`, 1 GPU, 8 CPUs, 48 GB, 6 h, three array tasks. Submit, poll, record utilisation; raise `--cpus-per-task` if the dataloader is the bottleneck.
- [ ] Evaluate on the 60 validation shots: against the Task 7a `active` labels AND against `annotated` (the raw pickle annotation, LFM excluded), frame AUROC / F1 / recall / frequency MAE, per loss. Against `annotated`, recall is the number that matters and low precision is expected (the annotations under-count); say so in the evaluation. Write `evaluation.json` and a 6-shot figure set. The SCE-vs-BCE gap is reported as evidence about label noise (spec section 5.6).
