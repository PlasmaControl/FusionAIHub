...
skips work that is already complete unless given `--force`. The 100-shot
proof-of-concept pool is `$LABELMAKER_ROOT/poc_shots.txt`.
...
100-shot proof-of-concept pool (all 100 aligned to their archived training rows;
6,400 published rows of 7,373 matched):
...
2. **Reconstruction fidelity** - our features against the model's own training
   ...
   costs 2.5e-3 (R0), 3.3e-3 (kappa), 3.4e-2 (1/qpsi); ZIPFIT standing in for
   the pipeline's own profile fits costs 6.7e-2 (ne), 1.25e-1 (Te), 1.4e-1
...
| `tm_prob` | AUROC | F1 at 0.5 | precision | recall |
|---|---|---|---|---|
| archived inputs | 0.932 | 0.581 | 0.436 | 0.870 |
| reconstructed inputs | 0.897 | 0.555 | 0.469 | 0.682 |

The gap - AUROC -0.035, `betan` RMSE 0.128 -> 0.149 - is the reconstruction
penalty, and the number that answers "is this reliable". The ceiling is scored
on rows the model trained on, so read it as optimistic; the honest reading is
the gap.
...
- ECH is on hold. The corpus has no deposition-location group, its
  ...
  outside the archive, any row with ECH power flowing is invalid.
