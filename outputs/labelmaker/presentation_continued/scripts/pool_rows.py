...
A copy of `../../presentation/scripts/pool_rows.py` parametrised by `--dsm-slug`
so the same figures can be rendered for the retrained survival checkpoint
(`d3d_tearing_time_to_event_dsm_continued`). The original is left untouched: it
is the record of what the Phase 2 figures were built from.

For every aligned shot: the archived truth, the tearing CNN scored twice (on
its own archived inputs and on labelmaker's reconstruction) and the chosen
survival model's three risks read back from the published labels, all on the
same matched rows.

    python pool_rows.py pool_rows.npz [--dsm-slug <slug>]
"""
import argparse
...
ap.add_argument("--dsm-slug", default="d3d_tearing_time_to_event_dsm_continued",
                help="which survival model's published labels to read")
args = ap.parse_args()
out = args.out
p = Paths.from_env()
