"""Figures 01-07 for outputs/labelmaker/presentation, both tearing models.

Reads pool_rows.npz (built by pool_rows.py over the 500-shot pool) plus the
validation JSON, and the label files for the two example shots.

    python make_presentation.py [--subset {all,held_out,in_training}]
                                [--out-dir <dir>] [--rows pool_rows.npz]

`--subset` restricts the whole pool to the shots the SURVIVAL model was or was
not trained on (214 of the 500 are training shots). It is a shot-level filter,
so every panel is drawn over the same shots; the CNN panels are in-sample
either way, because the archive scored against IS the CNN's training store.
Figure 04 is a pooled per-feature measurement with no subset of its own and is
drawn only for `--subset all`.
"""
import argparse
import json
import textwrap
...
ap.add_argument("--out-dir", default=str(SCR.parent), help="where the figures are written")
...
CNN, DSM = "d3d_tearing_onset_cnn1d", "d3d_tearing_time_to_event_dsm"
...
summary = {
    "subset": SUBSET, "subset_note": SUBSET_NOTE,
