#!/bin/bash
#SBATCH -A fus187
#SBATCH -J e2e_stage1_smoke_48L
#SBATCH -o logs/%j_e2e_stage1_smoke_48L.out
#SBATCH -e logs/%j_e2e_stage1_smoke_48L.err
#SBATCH -t 2:00:00
#SBATCH -p batch
#SBATCH -N 2
...
# 48-layer backbone smoke for Stage 1 (2026-05-20). Warm-starts from the
# current 26L production Stage 1.5 best via --init_checkpoint; the trainer
# auto-detects the 26→48 layer extension and initialises the 22 new blocks
# as near-identity (zero attn.out_proj + mlp final linear) so the deeper
# model emits the same outputs as the source until training wakes the new
# layers. Goal: measure peak VRAM %, step rate, and verify forward/backward
# survive at 2× depth. batch_size kept at production value (64) so the
# measurement applies directly to the production chain.
#
# Submit with: sbatch -q debug scripts/slurm_frontier/train_e2e_stage1_smoke_48L.sh
...
cd "${PROJECT_DIR}"
CHECKPOINT_DIR="/lustre/orion/fus187/proj-shared/models/e2e_stage1_smoke_48L"
...
if [ -f "${LATEST_CKPT}" ]; then
    echo "[train_e2e_stage1_smoke_48L] resuming from ${LATEST_CKPT}"
    RESUME_FLAG="--resume_checkpoint ${LATEST_CKPT}"
elif [ -f "${STAGE1_PROD_BEST}" ]; then
    echo "[train_e2e_stage1_smoke_48L] warm-starting 26→48L from ${STAGE1_PROD_BEST}"
    INIT_FLAG="--init_checkpoint ${STAGE1_PROD_BEST}"
else
    echo "ERROR: production Stage 1 best not found at ${STAGE1_PROD_BEST}." >&2
    echo "       48L smoke needs a 26L production checkpoint to warm-start." >&2
    exit 1
fi
