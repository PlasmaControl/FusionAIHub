#!/bin/bash
# Frontier DDP launcher: train_e2e Stage2 Extended — N nodes × 1 GCD (cross-node networking smoke; default N=2)
#
# Usage:
#   sbatch scripts/slurm_frontier/train_e2e_stage2_extended_Nx1.sh
#
...
#   sbatch -N 8 -t 12:00:00 scripts/slurm_frontier/train_e2e_stage2_extended_Nx1.sh
#
#SBATCH -A fus187
#SBATCH -J e2e_s2e_Nx1
#SBATCH -o logs/%j_e2e_s2e_Nx1.out
#SBATCH -e logs/%j_e2e_s2e_Nx1.err
#SBATCH -t 01:00:00
#SBATCH -p batch
#SBATCH -N 2
...
echo "${SMOKE_BANNER}[stage2_extended/Nx1] nodes=$NODES total_ranks=$TOTAL_RANKS \
batch=$BATCH_SIZE steps=$MAX_STEPS Ks=$CURRICULUM_KS"
echo "${SMOKE_BANNER}[stage2_extended/Nx1] master=$MASTER_ADDR:$MASTER_PORT data=$DATA_DIR"
