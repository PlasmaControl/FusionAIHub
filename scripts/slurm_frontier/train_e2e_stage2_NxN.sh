#!/bin/bash
# Frontier DDP launcher: train_e2e Stage2 — N nodes × 8 GCDs (production multi-node; default N=4, override with `sbatch -N <X>`)
#
# Usage:
#   sbatch scripts/slurm_frontier/train_e2e_stage2_NxN.sh
#
...
#   MASTER_PORT=<int>       # override port (default 29501)
#
# Override resource shape on the CLI (sbatch flags beat #SBATCH directives):
#   sbatch -N 8 -t 12:00:00 scripts/slurm_frontier/train_e2e_stage2_NxN.sh
#
#SBATCH -A fus187
#SBATCH -J e2e_s2_NxN
#SBATCH -o logs/%j_e2e_s2_NxN.out
#SBATCH -e logs/%j_e2e_s2_NxN.err
#SBATCH -t 02:00:00
#SBATCH -p batch
#SBATCH -N 4
...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-runs/e2e_stage2_frontier}"
...
if [ -f "$INIT_CHECKPOINT" ]; then
    ...
    echo "[stage2] init from $INIT_CHECKPOINT"
else
    echo "[stage2] WARNING: $INIT_CHECKPOINT not found — random init"
fi

NO_AMP_FLAG=""
[ "${NO_AMP:-0}" = "1" ] && NO_AMP_FLAG="--no_amp"
echo "${SMOKE_BANNER}[stage2/NxN] nodes=$NODES total_ranks=$TOTAL_RANKS \
batch=$BATCH_SIZE steps=$MAX_STEPS K_max=$K_MAX"
echo "${SMOKE_BANNER}[stage2/NxN] master=$MASTER_ADDR:$MASTER_PORT data=$DATA_DIR"
...
srun -N "$NODES" -n "$TOTAL_RANKS" -c "$CPUS_PER_TASK" \
     ...
     $INIT_FLAG $MAX_FILES_FLAG $NO_AMP_FLAG \
--data_dir "$DATA_DIR" \
