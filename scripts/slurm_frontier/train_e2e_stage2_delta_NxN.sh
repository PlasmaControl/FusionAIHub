#!/bin/bash
# Frontier DDP launcher: train_e2e Stage2 Delta — N nodes × 8 GCDs (production multi-node; default N=4, override with `sbatch -N <X>`)
#
# Usage:
#   sbatch scripts/slurm_frontier/train_e2e_stage2_delta_NxN.sh
#
...
#   sbatch -N 8 -t 12:00:00 scripts/slurm_frontier/train_e2e_stage2_delta_NxN.sh
#
#SBATCH -A fus187
#SBATCH -J e2e_s2d_NxN
#SBATCH -o logs/%j_e2e_s2d_NxN.out
#SBATCH -e logs/%j_e2e_s2d_NxN.err
#SBATCH -t 02:00:00
#SBATCH -p batch
#SBATCH -N 4
...
echo "${SMOKE_BANNER}[stage2_delta/NxN] nodes=$NODES total_ranks=$TOTAL_RANKS \
batch=$BATCH_SIZE steps=$MAX_STEPS K_max=$K_MAX"
echo "${SMOKE_BANNER}[stage2_delta/NxN] master=$MASTER_ADDR:$MASTER_PORT data=$DATA_DIR"
