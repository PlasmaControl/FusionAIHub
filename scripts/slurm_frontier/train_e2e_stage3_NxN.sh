#!/bin/bash
# Frontier DDP launcher: train_e2e Stage3 — N nodes × 8 GCDs (production multi-node; default N=4, override with `sbatch -N <X>`)
#
# Usage:
#   sbatch scripts/slurm_frontier/train_e2e_stage3_NxN.sh
#
...
#   sbatch -N 8 -t 12:00:00 scripts/slurm_frontier/train_e2e_stage3_NxN.sh
#
#SBATCH -A fus187
#SBATCH -J e2e_s3_NxN
#SBATCH -o logs/%j_e2e_s3_NxN.out
#SBATCH -e logs/%j_e2e_s3_NxN.err
#SBATCH -t 02:00:00
#SBATCH -p batch
#SBATCH -N 4
...
echo "${SMOKE_BANNER}[stage3/NxN] nodes=$NODES total_ranks=$TOTAL_RANKS \
batch=$BATCH_SIZE steps=$MAX_STEPS K=[$K_MIN,$K_MAX]"
echo "${SMOKE_BANNER}[stage3/NxN] master=$MASTER_ADDR:$MASTER_PORT data=$DATA_DIR"
