#!/bin/bash
# Frontier DDP launcher: train_e2e Stage1 — 1 node × 8 GCDs (production single-node DDP)
#
# Usage:
#   sbatch scripts/slurm_frontier/train_e2e_stage1_1x8.sh
#
...
#   sbatch -N 8 -t 12:00:00 scripts/slurm_frontier/train_e2e_stage1_1x8.sh
#
#SBATCH -A fus187
#SBATCH -J e2e_s1_1x8
#SBATCH -o logs/%j_e2e_s1_1x8.out
...
echo "${SMOKE_BANNER}[stage1/1x8] nodes=$NODES total_ranks=$TOTAL_RANKS \
batch=$BATCH_SIZE steps=$MAX_STEPS"
echo "${SMOKE_BANNER}[stage1/1x8] master=$MASTER_ADDR:$MASTER_PORT data=$DATA_DIR"
