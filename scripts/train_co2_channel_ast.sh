#!/bin/bash
#SBATCH --job-name=co2_chast_fw16
#SBATCH --output=logs/%j_co2_chast_fw16.out
#SBATCH --error=logs/%j_co2_chast_fw16.err
...
# Channel-AST, frame_width=16 — CO2 (C=4)
...
srun pixi run python scripts/training/spectrogram_reconstruction.py \
    ...
    --hop_length 128 \
    --log_interval 5 \
    --checkpoint_dir runs/co2_channel_ast_fw16 \
    --resume
