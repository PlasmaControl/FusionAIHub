"""Visualize CO2 Channel-AST (no FSQ, fw=8) reconstruction (shots 200000–200500).
...
# ── Config ────────────────────────────────────────────────────────────────────
CHECKPOINT = Path("runs/co2_channel_ast_nofsq_fw8/co2_spectrogram_channel_ast_fsq/checkpoint.pth")
...
# ── Build model (no FSQ, no merge, frame_width=8) ────────────────────────────
model = build_model("spectrogram_channel_ast_fsq", d_model=256, n_tokens=0,
                    n_channels=n_channels, freq_bins=freq_bins, frame_width=8,
...
# Token count for title
import math
n_frames = math.ceil(sample.shape[2] / 8)
...
fig.suptitle(
    f"CO2 Channel-AST (no FSQ, fw=8, {n_tokens_total} tokens) "
...
ax_loss.legend()
ax_loss.set_title(f"CO2 Channel-AST (no FSQ, fw=8, {n_tokens_total} tokens) — Loss Curves")
fig2.tight_layout()
