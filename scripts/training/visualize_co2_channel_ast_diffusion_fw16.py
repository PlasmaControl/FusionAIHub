"""Visualize CO2 Channel-AST-Diffusion (fw=16) reconstruction.
...
Usage:
    pixi run python scripts/training/visualize_co2_channel_ast_diffusion_fw16.py
"""
...
# -- Config -------------------------------------------------------------------
CHECKPOINT = Path("runs/co2_channel_ast_diffusion_fw16/co2_spectrogram_channel_ast_diffusion/checkpoint.pth")
...
model = build_model(
    ...
    d_model=256, n_tokens=0, n_channels=n_channels,
    freq_bins=freq_bins, frame_width=FRAME_WIDTH,
    time_conv_kernel=7, eval_steps=EVAL_STEPS,
)
...
fig.suptitle(
    f"CO2 Channel-AST-Diffusion (fw={FRAME_WIDTH}, {n_tokens_total} tokens, "
    f"{EVAL_STEPS} steps) — epoch {epoch + 1}, train MSE={final_loss:.4f}",
    fontsize=11,
)
fig.tight_layout()
...
ax_loss.plot(range(1, len(train_losses) + 1), train_losses,
             color="tab:blue", label="Train MSE (diffusion)")
if val_losses:
    ax_loss.plot(range(1, len(val_losses) + 1), val_losses,
                 color="tab:orange", label="Val L1 (reconstruction)")
ax_loss.set_xlabel("Epoch")
ax_loss.set_ylabel("Loss")
ax_loss.grid(True, alpha=0.3)
ax_loss.legend()
ax_loss.set_title(
    f"CO2 Channel-AST-Diffusion (fw={FRAME_WIDTH}, {n_tokens_total} tokens) — Loss Curves"
)
fig2.tight_layout()
