"""Visualize CO2 Channel-AST-Merge deep pool/expand (k=4, fw=8, 3L) reconstruction.
...
Usage:
    pixi run python scripts/training/visualize_co2_channel_ast_merge_deep.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
CHECKPOINT = Path("runs/co2_channel_ast_merge_k4_fw8_deep/co2_spectrogram_channel_ast_merge/checkpoint.pth")
...
# ── Build model ───────────────────────────────────────────────────────────────
model = build_model("spectrogram_channel_ast_merge", d_model=256, n_tokens=0,
                    n_channels=n_channels, freq_bins=freq_bins, frame_width=FW,
                    n_merge_queries=K, n_pool_layers=N_POOL_L,
                    n_expand_layers=N_EXPAND_L, time_conv_kernel=7)
...
fig.suptitle(
    f"CO2 Channel-AST-Merge deep (k={K}, fw={FW}, "
    f"{N_POOL_L}L pool/{N_EXPAND_L}L expand, {n_tokens_total} tokens) "
