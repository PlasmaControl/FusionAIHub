"""Visualize MHR Channel-AST (no FSQ, fw=16) reconstruction.

Usage:
    pixi run python scripts/training/visualize_mhr_channel_ast_nofsq_fw16.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
CHECKPOINT = Path("runs/mhr_channel_ast_nofsq_fw16_v2/mhr_spectrogram_channel_ast_fsq/checkpoint.pth")
...
hdf5_files = [
    ...
    if 200110 <= int(f.stem.split("_")[0]) <= 200500
]
...
for ch in range(n_channels):
    axes_row = axes[ch] if n_channels > 1 else axes
    orig_ch  = original[ch].numpy()
    recon_ch = reconstructed[ch].numpy()
    err_ch   = recon_ch - orig_ch

    axes_row[0].imshow(orig_ch,  cmap="viridis", origin="lower",
                       aspect="auto", vmin=vmin, vmax=vmax)
    axes_row[1].imshow(recon_ch, cmap="viridis", origin="lower",
                       aspect="auto", vmin=vmin, vmax=vmax)
    emax = max(abs(err_ch).max(), 1e-8)
    axes_row[2].imshow(err_ch,   cmap="bwr",     origin="lower",
                       aspect="auto", vmin=-emax, vmax=emax)

    for ax in axes_row:
        ax.set_xticks([])
        ax.set_yticks([])
    axes_row[0].set_ylabel(f"Ch {ch}", fontsize=9)

top_axes = axes[0] if n_channels > 1 else axes
top_axes[0].set_title("Original",      fontsize=11)
top_axes[1].set_title("Reconstructed", fontsize=11)
top_axes[2].set_title("Error (R − O)", fontsize=11)

fig.suptitle(
    f"MHR Channel-AST (no FSQ, fw={FW}, C={n_channels}, "
...
ax_loss.legend()
ax_loss.set_title(
    f"MHR Channel-AST (no FSQ, fw={FW}, C={n_channels}, {n_tokens_total} tokens)"
)
fig2.tight_layout()
