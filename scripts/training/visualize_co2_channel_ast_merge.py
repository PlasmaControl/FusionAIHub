"""Visualize CO2 Channel-AST-Merge (k=4, fw=8) reconstruction.
...
Usage:
    pixi run python scripts/training/visualize_co2_channel_ast_merge.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
CHECKPOINT = Path("runs/co2_channel_ast_merge_k4_fw8/co2_spectrogram_channel_ast_merge/checkpoint.pth")
...
model = build_model("spectrogram_channel_ast_merge", d_model=256, n_tokens=0,
                    ...
                    n_merge_queries=K, time_conv_kernel=7)
...
fig.suptitle(
    f"CO2 Channel-AST-Merge (k={K}, fw={FW}, {n_tokens_total} tokens) "
...
ax_loss.legend()
ax_loss.set_title(
    f"CO2 Channel-AST-Merge (k={K}, fw={FW}, {n_tokens_total} tokens) — Loss Curves"
)
fig2.tight_layout()
