"""Visualize CO2 MAE reconstruction vs. original for all channels.

Usage:
    pixi run python scripts/training/visualize_co2_mae.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
CHECKPOINT  = Path("runs/co2_mae/co2_spectrogram_mae/checkpoint.pth")
...
OUT_PATH    = Path("runs/co2_mae/co2_spectrogram_mae/plots/reconstruction_final.png")
# ─────────────────────────────────────────────────────────────────────────────
...
# Infer n_channels from a sample before building the model
...
model = build_model("spectrogram_mae", d_model=256, n_tokens=0,
                    n_channels=n_channels, mask_ratio=0.75)
...
for ch in range(n_channels):
    ...
    im0 = axes[ch, 0].imshow(orig_ch,  cmap="viridis", origin="lower",
                               aspect="auto", vmin=vmin, vmax=vmax)
    im1 = axes[ch, 1].imshow(recon_ch, cmap="viridis", origin="lower",
                               aspect="auto", vmin=vmin, vmax=vmax)
    im2 = axes[ch, 2].imshow(err_ch,   cmap="bwr",     origin="lower",
                               aspect="auto",
...
fig.suptitle(f"CO2 MAE — epoch {epoch + 1}, train L1={final_loss:.4f}", fontsize=12)
fig.tight_layout()
