"""Visualize CO2 FSQ-VAE p8 matched reconstruction (shots 200000–200500).
...
Usage:
    pixi run python scripts/training/visualize_co2_fsq_vae.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
CHECKPOINT = Path("runs/co2_fsq_vae_p8_matched/co2_spectrogram_fsq_vae/checkpoint.pth")
...
# ── Build model ───────────────────────────────────────────────────────────────
model = build_model("spectrogram_fsq_vae", d_model=256, n_tokens=0,
                    n_channels=n_channels, patch_h=8, patch_w=8)
...
fig.suptitle(
    f"CO2 FSQ-VAE p8 matched ({n_codes} codes) "
...
ax_loss.set_title("CO2 FSQ-VAE p8 matched — Loss & Codebook Utilisation")
fig2.tight_layout()
