"""Visualize CO2 AST-FSQ reconstruction (shots 200000–200500).
...
Usage:
    pixi run python scripts/training/visualize_co2_ast_fsq.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
CHECKPOINT = Path("runs/co2_ast_fsq_wide/co2_spectrogram_ast_fsq/checkpoint.pth")
...
# ── Build model ───────────────────────────────────────────────────────────────
model = build_model("spectrogram_ast_fsq", d_model=256, n_tokens=0,
                    n_channels=n_channels, freq_bins=freq_bins, frame_width=2, fsq_levels=[8,8,5,5,5,5,5])
...
fig.suptitle(
    f"CO2 AST-FSQ frame_width=2 ({n_codes} codes) "
...
ax_loss.set_title("CO2 AST-FSQ — Loss & Codebook Utilisation")
fig2.tight_layout()
