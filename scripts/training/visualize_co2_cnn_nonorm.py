"""Visualize CO2 CNN (no normalizer) reconstruction vs. original.

Ablation counterpart to visualize_co2_cnn_norm.py — same model architecture
but without the learned normalizer, using default log_standardize preprocessing.

Usage:
    pixi run python scripts/training/visualize_co2_cnn_nonorm.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
CHECKPOINT  = Path("runs/co2_cnn_nonorm/co2_spectrogram_cnn/checkpoint.pth")
...
HOP_LENGTH  = 128
CNN_DIMS    = [64, 128]
SAMPLE_IDX  = 10          # matches FSQ viz and normalized viz
SHOT_MIN    = 200000
SHOT_MAX    = 200500
...
final_loss = losses[-1]
fig.suptitle(f"CO2 CNN (no normalizer) — epoch {epoch + 1}, train L1={final_loss:.4f}",
             fontsize=12)
fig.tight_layout()
...
ax2.set_title("Training Loss Curve (no normalizer)")
ax2.legend()
