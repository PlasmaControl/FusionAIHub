...
class gadata:
    ...
    def __init__(self, signal, shot, tree=None, connection=None, nomds=False) -> None:
        # Save object values
        self.signal = signal
        self.shot = shot
        self.zdata = -1
        ...
        if not nomds:
            try:
                ...
                if numpy.rank(self.zdata) == 2:
                    self.zdata = numpy.transpose(self.zdata)
                if numpy.rank(self.ydata) == 2:
                    self.ydata = numpy.transpose(self.ydata)
                if numpy.rank(self.xdata) == 2:
                    self.xdata = numpy.transpose(self.xdata)

            except Exception:
                print(f"   Signal not in MDSplus: {signal}")
                pass
