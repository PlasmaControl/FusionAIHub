...
multi_level = ['cer', 'mag', 'profiles', 'basic', 'actu',
               'ts', 'ts_error', 'ts_rz']
...
file_keys = {'co2_s': ['r0', 'v1', 'v2', 'v3'],
             ...
             'cer': {output: [f'q.{output}.v{channel:02d}'
                             ...
                             [f'q.{output}.t{channel:02d}'
                              for channel in range(1, 49)]
                     for output in ['amp', 'samp', 'ti', 'sti', 'rot', 'srot',
                                    'r', 'phi', 'nz', 'fz', 'zeff', 'vb',
                                    'svb']},
             'mse': [r'%02d' % i for i in range(1, 70)],
             'mag': {'dsl': ['dsl.1u180', 'dsl.2u180', 'dsl.3u180',
                             'dsl.4u157', 'dsl.5u157', 'dsl.6u157'],
                     'mpi': [
                         'mpi.11m067', 'mpi.11m322', 'mpi.1a139', 'mpi.1a322',
                         'mpi.1b139', 'mpi.1b157', 'mpi.1b322', 'mpi.1l180',
                         'mpi.1u157', 'mpi.2a067', 'mpi.2a139', 'mpi.2a322',
                         'mpi.2b067', 'mpi.2b139', 'mpi.2b322', 'mpi.2l180',
                         'mpi.2u157', 'mpi.3a139', 'mpi.3a322', 'mpi.3b139',
                         'mpi.3b322', 'mpi.3l180', 'mpi.3u157', 'mpi.4a139',
                         'mpi.4a322', 'mpi.4b139', 'mpi.4b322', 'mpi.4u157',
                         'mpi.5a139', 'mpi.5a322', 'mpi.5b139', 'mpi.5b322',
                         'mpi.5u157', 'mpi.66m067', 'mpi.66m157', 'mpi.66m247',
                         'mpi.66m322', 'mpi.67a097', 'mpi.67a142',
                         'mpi.67a157', 'mpi.67a322', 'mpi.67b097',
                         'mpi.67b157', 'mpi.67b322', 'mpi.6fa322',
                         'mpi.6fb142', 'mpi.6fb322', 'mpi.6na132',
                         'mpi.6na157', 'mpi.6na322', 'mpi.6nb157',
                         'mpi.6nb322', 'mpi.6u157', 'mpi.79a147', 'mpi.79b142',
                         'mpi.79b322', 'mpi.79fa322', 'mpi.79na322',
                         'mpi.7fa322', 'mpi.7fb322', 'mpi.7na322',
                         'mpi.7nb142', 'mpi.7nb322', 'mpi.7u157', 'mpi.89a322',
                         'mpi.89b322', 'mpi.8a322', 'mpi.8b322', 'mpi.9a322',
                         'mpi.9b322'],
                     'psf': ['psf.1a', 'psf.1b', 'psf.2a', 'psf.2b', 'psf.3a',
                             'psf.3b', 'psf.4a', 'psf.4b', 'psf.5a', 'psf.5b',
                             'psf.6fa', 'psf.6fb', 'psf.6na', 'psf.6nb',
                             'psf.7fa', 'psf.7fb', 'psf.7na', 'psf.7nb',
                             'psf.8a', 'psf.8b', 'psf.9a', 'psf.9b'],
                     'psi': ['psi.11m', 'psi.12a', 'psi.12b', 'psi.1l',
                             'psi.23a', 'psi.23b', 'psi.2l', 'psi.34a',
                             'psi.34b', 'psi.3l', 'psi.45a', 'psi.45b',
                             'psi.58a', 'psi.58b', 'psi.6a', 'psi.6b',
                             'psi.7a', 'psi.7b', 'psi.89fb', 'psi.89nb',
                             'psi.9a', 'psi.9b']},
             ...
             'profiles': {'pressure': ['betap', 'betan', 'pres'],
                          ...
                          'q_rho_info': ['rhoqmin'],
                          'mag_geo_para': ['alpha', 'r0', 'aminor', 'kappa',
                                           'tritop', 'tribot', 'rmaxis',
                                           'zmaxis', 'volume'],
                          'mag_map': ['psirz', 'ssibry', 'ssimag'],
                          'divertor_geo': ['drsep', 'gapbot', 'gapin',
                                           'gapout', 'gaptop', 'zxpt1',
                                           'zxpt2'],
                          'profile': ['edensfit', 'etempfit', 'itempfit',
                                      'idensfit', 'trotfit'],
                          'mag_mode_number': ['n1rms', 'n2rms', 'n3rms']},
             'basic': {'mag': ['ip', 'ipsip', 'iptipp', 'pcbcoil', 'bcoil',
                               'bt', 'vloop'],
                       'neutron': ['plasticfix', 'fzns'],
                       'd_alpha': ['fs00', 'fs01', 'fs02', 'fs03', 'fs04',
                                   'fs05']},
             'actu': {'pinj': ['pinjf_%dl' % k for k in [15, 21, 30, 33]] +
                      ...
                      'rmp_current': ['c19', 'c79', 'c139', 'c199', 'c259',
                                      'c319', 'iu30', 'iu90', 'iu150', 'iu210',
                                      'iu270', 'iu330', 'il30', 'il90',
                                      'il150', 'il210', 'il270', 'il330'],
                      'coil_field_strength': ['ecoila', 'ecoilb', 'e567up',
                                              'e567dn', 'e89dn', 'e89up'] +
                                             ['f1a', 'f2a', 'f3a', 'f4a',
                                              'f5a', 'f6a', 'f7a', 'f8a',
                                              'f9a', 'f1b', 'f2b', 'f3b',
...
spec_params_default = {'window': 'hamm',
...
class DichargePerp():
    ...
    def file_path_gen(self, discharge, suffix):
        return (f'/scratch/gpfs/EKOLEMEN/big_d3d_data/'
                f'{str(discharge)[:2]}0000/{discharge}_{suffix}.h5')
    ...
    @staticmethod
    def spec_filters(freq, time, amp_f_t, spec_params=spec_params_default,
                     thr=0.9, gaussblr_win=(31, 3)):
        def norm(amp_f_t):
            mn = amp_f_t.mean()
            std = amp_f_t.std()
    ...
    @staticmethod
    def spectro_calc(sig_time, data, spec_params=spec_params_default,
                     plot=False):
