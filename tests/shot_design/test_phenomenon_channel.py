...
def test_the_channel_uses_the_existing_score_and_tier_order(shot_design_db):
...
def test_no_resolved_phenomenon_is_exactly_no_vote_in_rrf(shot_design_db, monkeypatch):
...
def test_the_channel_applies_the_full_hard_filter_before_its_candidate_limit(shot_design_db):
    db = _db_with(shot_design_db, [_event(shot, str(shot)) for shot in (100, 101, 200, 201)])
...
def test_resolution_denials_and_text_negatives_do_not_create_positive_hits(shot_design_db):
    db = _db_with(shot_design_db, [_event(100, 'seen')])
