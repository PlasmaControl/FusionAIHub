...
def test_every_source_that_ran_is_a_row_even_with_no_events(shot_file, paths,
    ...
    # this whole shot and the QH proxy finds no candidate in it, so
    # neither writes an event row - and both ran. (The proxy needs the
    # features store's `ip` before it can run at all; without one it is
    # SKIPPED rather than a successful zero, which is its own test.)
