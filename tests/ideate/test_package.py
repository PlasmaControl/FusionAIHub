...
def test_importing_the_module_entry_point_does_not_run_the_cli() -> None:
    ...
    `import ideate.__main__` -- which is what a doc tool, a coverage run or a reload does."""
    mod = importlib.import_module("ideate.__main__")
...
def test_python_dash_m_ideate_still_runs_the_cli() -> None:
    ...
    assert r.returncode == 0 and "usage: ideate" in r.stdout
